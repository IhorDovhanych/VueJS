...
const store = createStore({
    mutations: {
      addToBag(state,userId){
        for(let i = 0; i < state.itemsList.length; i++){
          if(userId == state.itemsList[i].id){
            let check = true; 
            if(state.bagList.length != 0){
              for(let j = 0; j < state.bagList.length; j++){
                if(userId == state.bagList[j].id){
                  state.bagList[j].count++;
                  check = false;
                  break;
                }
            ...
            if(check){
              state.bagList.push(state.itemsList[i])
            }
          }
        }
      },
      removeItemFromBag(state, userId){
        for(let i = 0; i < state.bagList.length; i++){
          if(userId == state.bagList[i].id){
            state.bagList.splice(i,1);
          }
        }
      },
      plusItemCountBag(state, userId){
        for(let i = 0; i < state.bagList.length; i++){
          if(userId == state.bagList[i].id){
      ...
      minusItemCountBag(state, userId){
        for(let i = 0; i < state.bagList.length; i++){
          if(userId == state.bagList[i].id){
            if(state.bagList[i].count > 0){
            state.bagList[i].count--;
          }
            break;
          }
    ...
    actions:{
      ...
      removeItemFromBag({commit},userId){
        commit("removeItemFromBag", userId)
      },
      plusItemCountBag({commit},userId){
        commit("plusItemCountBag", userId)
      },
      minusItemCountBag({commit},userId){
        commit("minusItemCountBag", userId)
      }
